...
export default function Board({
  ...
  const onPieceDrop = useCallback(
    (source: Square, target: Square, piece: string): boolean => {
      if (!isPiecePlayable({ piece })) {
        // If pre-move is supported, queue it instead of rejecting
        if (onPreMove && canPlay) {
          onPreMove(source, target);
          return true; // Piece visually drops but move is queued
        }
        return false;
      }
    ...
    [isPiecePlayable, playMove, onPreMove, canPlay]
  );
  ...
  const customArrows: Arrow[] = useMemo(() => {
    ...
    // Show up to 5 best moves with decreasing opacity green color
    if (showTopMovesArrows && position?.lastEval?.lines) {
      const topLines = position.lastEval.lines.slice(0, 5);
      const opacities = [0.85, 0.65, 0.45, 0.3, 0.15];
      topLines.forEach((line, index) => {
        ...
        if (move && move.length >= 4) {
          ...
          const greenColor = `rgba(34, 172, 56, ${opacity})`;
          arrows.push([fromSquare, toSquare, greenColor] as Arrow);
        }
      });
      return arrows;
    }
    ...
    return arrows;
  }, [position, showBestMoveArrow, showTopMovesArrows, boardHue]);
