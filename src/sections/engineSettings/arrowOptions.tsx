...
export default function ArrowOptions() {
  ...
  const [showTopMoves, setShowTopMoves] = useAtomLocalStorage(
    "show-arrows-top-moves",
    showTopMovesArrowsAtom
  );
      ...
      <FormControlLabel
        control={
          <Checkbox
            ...
            onChange={(_, checked) => {
              setShowTopMoves(checked);
              if (checked && multiPv < 5) {
                setMultiPv(5);
              }
            }}
          />
        }
        label="Show top 5 candidate moves (fading green)"
        sx={{ marginX: 0 }}
      />
      <FormControlLabel
        control={
