...
export const getLichessUserAllRecentGames = async (
  username: string,
  maxGames: number | undefined = 50,
  signal?: AbortSignal
): Promise<LoadedGame[]> => {
  const maxParam = maxGames === undefined ? "" : `&max=${maxGames}`;
  const res = await fetch(
    `https://lichess.org/api/games/user/${username}?until=${Date.now()}${maxParam}&pgnInJson=true&sort=dateDesc&clocks=true&opening=true`,
    { method: "GET", headers: { accept: "application/x-ndjson" }, signal }
  );
