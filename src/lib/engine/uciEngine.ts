...
export class UciEngine {
  ...
  public async evaluateGame({
    ...
    try {
      ...
      if (signal) {
        try {
          await Promise.race([evaluationPromise, abortPromise]);
        } finally {
          if (abortListener) {
  ...
  public async getEngineNextMove(
    ...
    // Determine target depth dynamically based on Elo
    ...
    // Set Elo on the engine
    await this.setElo(elo);

    // If Elo is below 1320, we occasionally blunder (play a random legal move)
    if (elo < 1320) {
